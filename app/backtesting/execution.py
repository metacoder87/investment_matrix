...
@dataclass
class ShortCoverResult(FillResult):
    pnl: float
...
def compute_short_open_fill(cash: float, equity: float, price: float, settings: ExecutionSettings) -> FillResult | None:
    if cash <= 0 or price <= 0:
        return None
    ...
    qty = target_value / fill_price
    fee = qty * fill_price * settings.fee_rate
    total_reserved = qty * fill_price + fee
    ...
    return FillResult(
        ...
        fee=fee,
        notional=qty * fill_price,
        cash_delta=-total_reserved,
    )


def compute_short_cover_fill(
    position_qty: float,
    entry_price: float,
    reserved_collateral: float,
    price: float,
    settings: ExecutionSettings,
) -> ShortCoverResult | None:
    if position_qty <= 0 or entry_price <= 0 or price <= 0:
        return None
    ...
    notional = position_qty * fill_price
    fee = notional * settings.fee_rate
    pnl = (entry_price - fill_price) * position_qty
    cash_delta = reserved_collateral + pnl - fee
